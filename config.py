...
class MarketDataParams:
    ...
    """
    # Historical data
    historical_days: int = 100             # 100 days for pattern analysis
    minimum_candles_required: int = 50     # Need minimum 50 candles for reliability
    
    # Data intervals
    primary_interval: str = "day"          # Main analysis interval
    supported_intervals: List[str] = field(
        default_factory=lambda: ["1minute", "5minute", "15minute", "30minute", "day", "week", "month"]
    )
...
class APICredentialsParams:
    ...
    def validate(self) -> bool:
        ...
        if not self.upstox_access_token:
            raise ValueError("UPSTOX_ACCESS_TOKEN not configured")
        return True
...
class BotConfiguration:
    ...
    patterns: CandlestickPatternThresholds = field(default_factory=CandlestickPatternThresholds)
    ...
    telegram: TelegramNotificationParams = field(default_factory=TelegramNotificationParams)
    ...
    def validate_all(self) -> Dict[str, Any]:
        ...
        if len(self.stocks_to_monitor) > 50:
            warnings.append(f"Monitoring {len(self.stocks_to_monitor)} stocks - may impact performance")
        ...
        if self.market_data.historical_days < 50:
            warnings.append(f"historical_days = {self.market_data.historical_days} is too low, recommend 100+")
        ...
        if self.monitoring.dashboard_update_frequency_seconds < 10:
            warnings.append("Dashboard update frequency <10s may cause performance issues")
    ...
    def from_dict(cls, data: Dict) -> 'BotConfiguration':
        ...
        # This is simplified - production version would handle nested dataclasses
        return cls(**data)
...
def load_config_from_environment() -> BotConfiguration:
    ...
    Environment variables follow pattern: BOT_<SECTION>_<PARAM>
    ...
    # Load min RRR from environment (overrides default)
